package com.example.readers.readermanagement.infraestructure.repositories.impl;
...
public interface SpringDataReaderRepositoryImpl extends ReaderRepository, ReaderDetailsRepoCustom, CrudRepository<ReaderDetails, Long> {
    ...
    @Override
    @Query("SELECT NEW com.example.readers.readermanagement.services.ReaderBookCountDTO(rd, count(l)) " +
            "FROM ReaderDetails rd " +
            "JOIN Lending l ON l.readerDetails.pk = rd.pk " +
...
class ReaderDetailsRepoCustomImpl implements ReaderDetailsRepoCustom {
    ...
    @Override
    public List<ReaderDetails> searchReaderDetails(final com.example.readers.shared.services.Page page, final SearchReadersQuery query) {
